...
$(document).ready(function () {
    ...
    $(".modal-close").on("click", function (APIkey3) {
        ...
        results = $("#search-results").val().trim();
    ...
    function displayUv() {
        ...
        if (debugMode === false && city !== cityDebug) {
            ...
            console.log("debugResponse2");


            createForecastDebug();

            function createForecastDebug() {
                // clearing the display divs for the 5 day forecast cards
                if (cardsFull === true) {
                    for (var i = 1; i < 6; i++) {
                        $(datesArrey[i]).empty();
                ...
                for (var i = 1; i < 6; i++) {
                    var date1 = $("<p>");
                    var dateDU1 = debugResponse1.daily[i].dt;
                    var dateD1 = new Date();
                    dateD1.setTime(dateDU1 * 1000);
                    ...
                    var weatherIcon = debugResponse1.daily[i].weather[0].icon;
                    ...
                    // creating values for humidity and temp
                    var humidityD = debugResponse1.daily[i].humidity;
                    var tempK = debugResponse1.daily[i].temp.day;
                    var tempF = (tempK - 273.15) * 1.80 + 32;
                    var pTwo = $("<p>").text("Humidity:" + humidityD + "%");
                    var pThree = $("<p>").text("Temp: " + tempF.toFixed(1) + " F");
                    // appending to the card
    ...
    function findBikeTrails(lon, lat) {
        ...
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "https://trailapi-trailapi.p.rapidapi.com/trails/explore/?page=" + pages + "&per_page=" + perPage + "&radius=" + radius + "&lat=" + lat + "&lon=" + lon,
            "method": "GET",
            "headers": {
        ...
        if (debugMode === false && city !== cityDebug) {
            ...
            for (var i = 0; i < debugResponse3.data.length; i++) {
                ...
                var aDirections = debugResponse3.data[i].directions;
                var aDescription = debugResponse3.data[i].description;
                var aLength = debugResponse3.data[i].length;
